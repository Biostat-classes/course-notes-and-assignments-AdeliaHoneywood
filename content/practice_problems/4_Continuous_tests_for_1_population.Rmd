...
H0 mu=161\
HA mu does not equal 161

```{r}
z.test(flies, mu=161, sigma.x=275) #fails to reject null hypothesis

```

```{r}
t.test(flies, mu=161) #fails to reject null hypothesis
```

```{r}
wilcox.test(flies, mu=161) #fails to reject null hypothesis
```

```{r}
SIGN.test(flies, md=161)#fails to reject null hypothesis, however 161 is in the confidence interval
```
...
H0 mu=205\
HA mu does not equal 205

```{r}
strawberry_yields<-c(239, 176, 235, 217, 234, 216, 318, 190, 181, 225)
```

```{r}
z.test(strawberry_yields, mu=205, sigma.x=1500) #fails to reject the null hypothesis
```

```{r}
t.test(strawberry_yields, mu=205) #fails to reject the null hypothesis
```

```{r}
wilcox.test(strawberry_yields,mu=205) #fails to reject null hypothesis but p-value is not exact
```

```{r}
SIGN.test(strawberry_yields,md=205) #fails to reject null hypothesis
```
...
```{r}
binom.test(25, 30, p=.5)#p-value <.05 rejects the null hypothesis that the sex ratio will be 50% female
```

```{r}
binom.test(25,30, p=.8)#p-value>.05 fails to reject hypothesis that the sex ratio will be 80% female
```
...
```{r}
snake_undulation_rate<-c(0.9, 1.4, 1.2, 1.2, 1.3, 2.0, 1.4, 1.6)
```

```{r}
t.test(snake_undulation_rate, mu=1.4)
```

I could not use a z-test because I did not know the standard deviation,
therefore I chose a t-test. The p-value \> .05 so the t-test failed to
reject the null hypothesis that the mean is 1.4 HZ. I did not use a
binomial test because the data is continuous.
...
The P value is less than .05 so we can reject the null hypothesis that
they don't differ in weight and say that they do differ in weight than
the average male population. The mean of the sample estimate is less
than the average weight of the overall population.
