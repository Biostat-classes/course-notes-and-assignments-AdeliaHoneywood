...
H0 people are not eared or proportion of people who are right-eared
equals .5

HA proportion of right eared people does not equal .5
...
hist(sampling_experiment, xlab="# Right-eared People in Sample", ylab="Probability of Right-earedness in Population if p parameter = .5")
```
...
```{r}
binom.confint(19, 25, conf.level=.95, methods="ac")
```

```{r}
binom.confint(19, 25, conf.level=.95, methods="all")
```

P-value is less than .05 so we can reject null hypothesis. Agresti-coull
confidence interval (.562 - .888 )does not include .05, but it does
include our observed proportion of .76
...
H0 p=.25 the students helped the dog

HA p\>.25 the students did not help the dog

```{r}
binom.test(33,100, alternative="greater", p=.25)
```

P value is less than .05, so I reject the null hypothesis that the
