...
This practice reviews the [Estimation lecture](../chapters/Estimation.qmd) and the use of ggplots (lots of ggplot2 example code in the [Summarizing data lecture](../chapters/Summarizing_data.qmd)
...
Next we add data layers using geom\_ commands. Let's start with a scatter plot, which we make using the geom_point command.
...
You can also directly change legend title and colours with the scale\_ commands
...
In general scale\_\[whatever you had aes commands\]\_manual lets you set colors or codes. To see color codes go to [this chart](http://sape.inf.usi.ch/quick-reference/ggplot2/colour){target="_blank"}
...
```{r}
TotalSleep_BrainWt_relationship <- ggplot(sleep[sleep$BrainWt <1000, ], aes(x=BrainWt, y = TotalSleep))
```

```{r}
TotalSleep_BrainWt_relationship
```

```{r}
TotalSleep_BrainWt_relationship_scatter <- ggplot(sleep[sleep$BrainWt <1000, ], aes(x=BrainWt, y = TotalSleep))+geom_point()
```

```{r}
TotalSleep_BrainWt_relationship_scatter
```
...
```{r}
ggplot(sleep, aes(x=BrainWt, y = TotalSleep)) +
  geom_point(aes(colour=Taxa), size = 2)+
...
```{r}
ggplot(sleep, aes(x=TotalSleep)) +
  geom_histogram(aes(colour=Taxa))+
...
ggplot(sleep, aes(x=TotalSleep)) +
  geom_histogram(aes(colour=Taxa))+
  theme(plot.background=element_rect(fill=taxa))
#trying to figure out how to fill the bars instead of having them outlined in color only
```
...
ggplot(sleep[sleep$Taxa == "Primate",]
       , aes(x=TotalSleep)) +
 geom_histogram(aes(colour=Taxa))+
xlab("Total sleep (hours per day)")+ggtitle("Variation in sleep levels for primates")+
  ...
  theme(axis.title.x = element_text(face="bold", size=15), 
        ...
        legend.title = element_text(size=12, face="bold"),
        plot.title = element_text(hjust = 0.5, face="bold", size=20), legend.position = "none")
```
...
```{r}
sleep$Taxa <- revalue(sleep$Primate, c(Y="Yes", N="No"))
```
...
ggplot(sleep_by_primate
  ...
  geom_col(size = 3) +
  geom_errorbar(aes(ymin=TotalSleep-ci, ymax=TotalSleep+ci), size=1.5) +
  xlab("Is Primate?")+ggtitle("Sleep in primates and non-primates")+
...
It means that for every sample taken, the true mean of the whole population will fall within this calculated interval 95% of the time.
...
For this question you'll need the central_limit_theorem.R script from the [code_examples folder](https://github.com/jsgosnell/CUNY-BioStats/tree/master/code_examples). Download it to your computer and open it. Alternatively, go ahead and make a copy of the CUNY-Biostats repository. You won't have write access but can keep one up-to-date on your machine/cloud (**pull** occassionally!).
