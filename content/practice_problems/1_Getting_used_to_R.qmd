...
The : outputs everything in between the two numbers, i.e. the list
...
max(x) returns the maximum value in the list

```{r}
x[x<10]
```

x\[x\<10\] outputs the elements of x that are less than 10

```{r}
x^2
```

x\^2 outputs each value of x squared

```{r}
x[x<12&x>7]
```

x\[x\<12&x\>7\] outputs the values of x that are greater than 7 and less
than 12, i.e. between and not including 7 and 12.
...
-1:2 is the same as (-1):2 but different than -(1:2). I guess in the
latter the algebraic rule of distributing the negative throught the
parentheses applies
...
The sleep data set has 62 rows and 12 variable columns. The variables
include species name, body weight, brain weight, sleep time when not
dreaming, sleep time while dreaming, total sleep time, life span,
gestation time, how likely to be predated upon while asleep, how exposed
to danger in sleep, and whether or not they are a primate
...
```{r}
colnames(sleep)[BodyWt]<-"Body_weight"
```
...
```{r}
plot(factor(Primate), TotalSleep)
```
